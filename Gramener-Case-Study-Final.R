...
#Sub_grade will not give us any more business impact than the one obtained from grade.
str(loan$sub_grade)
loan$sub_grade <- factor(loan$sub_grade)
table(loan$sub_grade)
loan <- loan[ , -which(colnames(loan) == "sub_grade")]
...
# Bivariante analysis of loans defaulted vs amount of loan. The trends indicate that loan defaulted and not defaulted
...
# Bi-variante analysis between open_acc and Has_Defaulted does not show any specific trend.
...
# of customers
ggplot(loan, aes(x = loan$verification_status, fill = loan$Has_Defaulted)) + geom_bar() + theme_bw() + labs(title = "Verification Status vs Count", x = "Verification Status", y = "Count")
...
ggplot(loan, aes(x = loan$purpose, fill = loan$Has_Defaulted)) + geom_bar()
...
loan$year = substr(loan$earliest_cr_line, start = 3, stop = 4)
ggplot(loan, aes(x = loan$year, fill = loan$Has_Defaulted)) + geom_bar()
...
# Trivariant analysis shows that applicants default more with high interest rates for 60 months tenure.
ggplot(loan, aes(x = loan$int_rate_category, fill = loan$Has_Defaulted)) + geom_bar() + facet_wrap(~loan$term)

# Trivarinant analysis shows that applicants default more with grades B, C and D for 60 months tenure.
ggplot(loan, aes(x = loan$grade, fill = loan$Has_Defaulted)) + geom_bar() + facet_wrap(~loan$term)

# Trivariant analyis shows that medium interest rate applicants default more for mortgage loans
ggplot(loan, aes(x = loan$int_rate_category, fill = loan$Has_Defaulted)) + geom_bar() + facet_wrap(~loan$home_ownership)

# Trivariant analysis for loan purpose vs interest rate vs has defaulted does not show any alarming trend.
ggplot(loan, aes(x = loan$purpose, fill = loan$Has_Defaulted)) + geom_bar() + facet_wrap(~loan$revol_util_category)

# Trivariant analysis for issue year vs has defaulted vs term shows that there are no 36 months loans taken in 2007, 2008 and 2009
ggplot(loan, aes(x = factor(loan$issue_year) , fill = loan$Has_Defaulted)) + geom_bar() + facet_wrap(~loan$term)+ theme_bw() + labs(title = "Issue year vs term vs Defaulted Customers", x = "Loan Issue Year", y = "Count")

# Bivarinat analysis of term vs has defaulted shows that there are more applicants who default on a 6 year term.
ggplot(loan, aes(x = factor(loan$term) , fill = loan$Has_Defaulted)) + geom_bar()

# Trivariant analysis of verification status vs loan defaulters vs issue year indicates that in 2011 there is high number of loans
# and there are more defaulters even though their source was verified and income was verified.
ggplot(loan, aes(x = loan$verification_status, fill = Has_Defaulted)) + geom_bar() + facet_wrap(~loan$issue_year)+ theme_bw() + labs(title = "Verification status vs Issue year vs Has Defaulted", x = "Verification Status", y = "Count")
...
# Write the file to excel in order to plot charts in Tablaeu
write.csv(loan, file = "Gramener_from_R.csv", row.names = FALSE)